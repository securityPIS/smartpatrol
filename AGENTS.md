...
Dependensi: SYSTEM_MAP.md, Firebase rules, arsitektur React + Vite + Firebase.
...
4. menghindari perubahan yang memperberat sinkronisasi, menambah biaya Firebase, atau membuka celah keamanan
...
`Trigger/Entry Point -> Handler/Controller -> Business Logic/Service -> Data Access -> Firebase/Storage/Local Persistence`
...
2. Perubahan pada log patroli, foto, assignment, atau status tugas harus dievaluasi terhadap dampaknya ke local state, IndexedDB, localStorage, dan sinkronisasi ke Firebase.
...
3. Pertimbangkan ukuran payload gambar, frekuensi sync, dan biaya Firestore/Storage pada perangkat mobile.

## Standar Security dan Integritas Data

1. Jangan menganggap rule Firebase saat ini sudah aman; selalu review kebutuhan otorisasi secara eksplisit.
2. Setiap perubahan yang menyentuh Firestore, Storage, Auth, atau role harus mengecek dampak ke RBAC.
...
## Standar Firestore dan Query

Repo ini memakai Firebase, jadi evaluasi performa harus fokus ke karakteristik Firestore, bukan asumsi SQL tradisional.
