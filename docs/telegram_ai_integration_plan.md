...
4. **Firebase Secrets**: Untuk menyimpan Token dan API Key secara aman.
...
Bot AI akan diintegrasikan menggunakan **Firebase Cloud Functions (Webhook)**.

- **Mekanisme**: Telegram Webhook -> Firebase Cloud Function -> AI API (Gemini) -> Telegram SendMessage.
- **Kemampuan AI**:
...
- Set `firebase functions:secrets:set TELEGRAM_BOT_TOKEN`.
- Set `firebase functions:secrets:set GEMINI_API_KEY`.
