...
import { useApp, defaultLocationOptions } from '../../context/AppContext';
import { ChevronDown, Camera, X, MapPin, Save } from 'lucide-react';
import AsyncImage from '../AsyncImage';
import { useFocusTrap } from '../../hooks/useFocusTrap';

export default function IncidentFormModal() {
  const { showIncidentModal, incidentForm, setIncidentForm, incidentLocationOptions, closeIncidentModal, handleSubmitIncident, handlePhotoUpload } = useApp();
  const modalRef = useFocusTrap(showIncidentModal);
  if (!showIncidentModal) return null;
  const locs = incidentLocationOptions;

  return (
    <div ref={modalRef} className="fixed inset-0 z-[100] bg-[#070b19] sm:max-w-md sm:mx-auto sm:border-x sm:border-yellow-500/20 flex flex-col animate-in slide-in-from-right-4">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div><label className="text-[10px] uppercase tracking-widest text-cyan-500 mb-1.5 block font-bold">Penyebab Kejadian</label><textarea rows={2} value={incidentForm.penyebab} onChange={e => setIncidentForm(prev => ({...prev, penyebab: e.target.value}))} placeholder="Apa indikasi penyebabnya..." className="w-full bg-[#0b1229] border border-cyan-800/50 rounded-xl p-3 text-sm text-cyan-50 focus:border-yellow-500 outline-none resize-none" /></div>
        <div><label className="text-[10px] uppercase tracking-widest text-cyan-500 mb-1.5 block font-bold">Tindak Lanjut Awal</label><textarea rows={2} value={incidentForm.tindakLanjut} onChange={e => setIncidentForm(prev => ({...prev, tindakLanjut: e.target.value}))} placeholder="Tindakan awal yang sudah dilakukan..." className="w-full bg-[#0b1229] border border-cyan-800/50 rounded-xl p-3 text-sm text-cyan-50 focus:border-emerald-500 outline-none resize-none" /></div>
      </div>
      <div className="p-4 bg-[#0b1229] border-t border-cyan-900/50 shrink-0 pb-safe flex gap-3">
        ...
        <button onClick={handleSubmitIncident} disabled={!(incidentForm.locType === 'custom' ? incidentForm.customLocation.trim() : incidentForm.location.trim()) || !incidentForm.deskripsi.trim()} className="flex-1 py-4 rounded-xl font-black tracking-widest uppercase text-xs bg-yellow-600 hover:bg-yellow-500 text-black disabled:opacity-50 transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(250,204,21,0.25)]"><Save className="w-4 h-4" /> Simpan</button>
      </div>
    </div>
  );
}
