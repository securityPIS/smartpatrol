...
const ShipsPage = React.memo(function ShipsPage() {
  const {
    ...
    setShowShipForm, shipDetailTab, setShipDetailTab, scheduleMonth, setScheduleMonth,
    ...
    handleChangeSchedule, handleDownloadShipDoc, openShipDocForm, newShipCp, setNewShipCp,
          ...
          {shipsData.map((ship) => (
            <div key={ship.id} onClick={() => setActiveShipId(ship.id)} className="bg-[#0b1229] rounded-xl border border-cyan-800/50 hover:border-cyan-400 transition-colors cursor-pointer flex overflow-hidden h-28 shadow-sm">
               <div className="w-28 h-full bg-[#070b19] border-r border-cyan-900 flex-shrink-0 relative">
                 <AsyncImage src={ship.photoUrl} alt={ship.name} className="w-full h-full object-cover opacity-80" />
               </div>
  ...
  return (
    <div className="p-4 space-y-4 animate-in slide-in-from-right-4 pb-10">
      ...
      <div className="h-40 rounded-2xl overflow-hidden relative border border-cyan-700 shadow-[0_0_20px_rgba(6,182,212,0.15)] group">
         ...
         <button onClick={() => { setActiveShipId(null); setIsEditingShipInfo(false); }} className="absolute top-3 left-3 bg-[#0b1229]/80 p-2 rounded-full border border-cyan-500/50 text-cyan-300 hover:text-white backdrop-blur-sm z-10" aria-label="Kembali"><ChevronDown className="w-5 h-5 rotate-90"/></button>
